...
var L = (typeof window !== "undefined" ? window.L : typeof global !== "undefined" ? global.L : null),
  ...
  L.fs_sleep = function(ms) {
    return new Promise(resolve => setTimeout(resolve, ms));
  },

  L.fs_moveMarker = async function(_marker, src, dest, count=250, sleep_time=30)
  {
    var coor=src;
    var lat_diff=dest.lat-src.lat;
    var lng_diff=dest.lng-src.lng;
    count=parseFloat(count);
    for(var i=1.0;i<count;i++)
    {
      coor.lat+=(1.0/count)*lat_diff;
      coor.lng+=(1.0/count)*lng_diff;
      _marker.setLatLng(coor);
      await L.fs_sleep(sleep_time);
    }
    await L.fs_sleep(sleep_time);
    _marker.setLatLng(dest);
  },

  /*L.fs_moveMarker = async function(marker, start, end, duration) {
        window._t = new Date();
        var dist = 1, __t = _t,
            distance = start.distanceTo(end),
            factor = distance / dist,
            interval = Math.floor(duration / factor),
            dLat = (end.lat - start.lat) / factor,
            dLng = (end.lng - start.lng) / factor;

        for(var i = 0; i < Math.floor(factor); i++)
        {
            if (window._old != undefined) { 
                start = window._old; 
                window._old = undefined; 
                distance = start.distanceTo(end);
                factor = distance / dist;
                interval = Math.floor(duration / factor);
                dLat = (end.lat - start.lat) / factor;
            ...
            if (window._t.getTime() !== __t.getTime()) { window._old = end; break; }
            start = new L.LatLng(start.lat + dLat, start.lng + dLng);
            marker.setLatLng(start);
            await L.fs_sleep(interval);
        }
    ...
    options: {
        ...
        updateFeature: function(f, oldLayer, newLayer) {
            if (f.geometry.type === 'Point') {
                if (!oldLayer.getLatLng().equals(newLayer.getLatLng()) || oldLayer.feature.properties.date_time != newLayer.feature.properties.date_time || oldLayer.feature.properties.status != newLayer.feature.properties.status) {

                  var dis = oldLayer.getLatLng().distanceTo(newLayer.getLatLng());
                  var difDis = Math.abs((newLayer.feature.properties.speed/3.6)*10 - dis);
                  var difTime = F.DateDifferance(oldLayer.feature.properties.date_time, newLayer.feature.properties.date_time);

                  if ((newLayer.feature.properties.speed == 0 && dis > 0) || dis > 400) {
                    console.log("error");
                  } else if (difDis > 50 && difTime == 10000)  {
                    console.log("warning");
                    if (difDis > 50 && difTime > 10000)
                      oldLayer.setLatLng(newLayer.getLatLng());
                  } else {
                    console.log("ok");
                    if (this.smoothly === true && newLayer.feature.properties.speed > 0) {
                      L.fs_moveMarker(oldLayer, oldLayer.getLatLng(), newLayer.getLatLng());
                    } else
                      oldLayer.setLatLng(newLayer.getLatLng());
                  }
                  console.log(newLayer.feature.properties.name, newLayer.feature.properties.speed, dis, difDis, difTime);
                  oldLayer.feature.properties = newLayer.feature.properties;
                  oldLayer.options.updated = true;
                }
                return oldLayer;
            } else {
                return newLayer;
    ...
    _onNewData: function(removeMissing, response) {
        ...
        this.options.onEachFeature = L.bind(function onEachFeature(f, l) {
            ...
            if (oldLayer) {
                ...
                l = newLayer;
                if (newLayer.options.updated == true) {
                  update[fId] = f;
                  newLayer.options.updated = false;
                }
            } else {
                enter[fId] = f;
            }
