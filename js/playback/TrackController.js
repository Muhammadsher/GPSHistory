...
L.Playback.TrackController = L.Class.extend({
    ...
    tock : function (timestamp, transitionTime) {
        ...
        for (var i = 0, len = this._tracks.length; i < len; i++) {
            ...
            var speedArray = this._tracks[i]._geoJSON.properties.speed;
            var time = this._tracks[i]._geoJSON.properties.time;
            var id = this._tracks[i]._geoJSON.device_id;  

            this._tracks[i].moveMarker(latLng, transitionTime, speedArray, time, id);
        }
        window.timeCurr=Curr;
