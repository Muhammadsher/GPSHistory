...
L.Playback.MoveableMarker = L.Marker.extend({    
    ...
    move: function (latLng, transitionTime, speedArray, time, id) {
       var myTime= window.timeCurr;
       
       for(var i=0; i< speedArray.length; i++){
      
            if(speedArray[i]>100&&time[i]==myTime){   
                document.getElementById('speed.'+id).className='red'; 

            }else if(transitionTime==0){
         
            }else{
                if(document.getElementById('speed.'+id).className='red'){
                 document.getElementById('speed.'+id).className='limit';
                }
                else{
